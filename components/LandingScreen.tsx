...
const LandingScreen: React.FC<Props> = ({ onNewGame, onResume, saveSummary, isMuted, toggleAudio }) => {
  return (
    <div className="h-full w-full bg-black text-white relative overflow-y-auto custom-scrollbar font-sans">
      {/* Background Grid and Scanlines */}
      <div className="fixed inset-0 bg-[linear-gradient(rgba(0,255,0,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(0,255,0,0.03)_1px,transparent_1px)] bg-[size:30px_30px] pointer-events-none"></div>
      <div className="fixed inset-0 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.1)_50%)] bg-[length:100%_4px] pointer-events-none opacity-40"></div>
      <div className="fixed top-0 left-0 w-full h-1 bg-green-500/20 animate-scan pointer-events-none"></div>
      ...
      <div className="min-h-full flex items-center justify-center p-4 md:p-8">
        <div className="max-w-5xl w-full z-10 grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center py-8">
            ...
            >
            <div>
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded border border-green-500/30 bg-green-900/10 text-green-400 text-[10px] md:text-xs font-mono mb-4">
                <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                SYSTEM_ONLINE // V2.0
                </div>
                <h1 className="text-4xl sm:text-5xl md:text-7xl font-black font-mono tracking-tighter leading-tight mb-2">
                SCAM<br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-500 to-emerald-300">SIMULATOR</span>
                </h1>
                <p className="text-base md:text-xl text-zinc-400 font-mono">The Social Engineering RPG</p>
            </div>
            ...
            <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-6 md:p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)] relative overflow-hidden group">
                ...
                <div className="flex items-center gap-3 mb-6">
                    <div className="w-10 h-10 md:w-12 md:h-12 bg-zinc-900 rounded-lg flex items-center justify-center border border-zinc-700 shrink-0">
                        <Terminal className="text-green-500" size={20} />
                    </div>
                    <div>
                        <h2 className="text-lg md:text-xl font-bold text-white font-mono">ACCESS CONTROL</h2>
                        <p className="text-[10px] md:text-xs text-zinc-500 uppercase tracking-widest">Identity Verification</p>
                    </div>
                </div>
                    ...
                    <div className="space-y-4">
                        <div className="p-4 bg-green-900/10 border border-green-500/30 rounded-lg">
                            ...
                            <div className="flex justify-between items-center mb-1">
                                <span className="text-white font-bold font-mono text-sm md:text-base truncate max-w-[150px]">{saveSummary.name}</span>
                                <span className="text-green-400 font-mono text-sm md:text-base">${saveSummary.money.toLocaleString()}</span>
                            </div>
                        ...
                        <button 
                            onClick={onResume}
                            className="w-full py-3 md:py-4 bg-green-600 hover:bg-green-500 text-black font-bold rounded-lg transition-all flex items-center justify-center gap-2 shadow-[0_0_20px_rgba(34,197,94,0.3)] hover:shadow-[0_0_30px_rgba(34,197,94,0.5)] hover:scale-[1.02] text-sm md:text-base"
                        >
                        ...
                        <button 
                            onClick={onNewGame}
                            className="w-full py-3 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-400 hover:text-white font-bold rounded-lg transition-all flex items-center justify-center gap-2 text-xs md:text-sm"
                        >
