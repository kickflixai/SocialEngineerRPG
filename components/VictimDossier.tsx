...
const VictimDossier: React.FC<Props> = ({ victim, player, onExecute, onAbort, loading }) => {
  ...
  return (
    <div className="h-full flex items-center justify-center p-8 bg-black/40">
      <div className="max-w-6xl w-full bg-zinc-950 border border-zinc-800/80 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col md:flex-row h-[85vh] relative">
        {/* CRT Line */}
        ...
        {/* Left: Photo & Basic ID */}
        <div className="md:w-1/3 bg-black/60 p-8 border-r border-zinc-800 flex flex-col items-center relative overflow-y-auto custom-scrollbar">
           <div className="relative w-56 h-56 mb-6 rounded-xl overflow-hidden border-2 border-zinc-700 shadow-[0_0_30px_rgba(0,255,0,0.1)] group shrink-0">
           ...
           <h2 className="text-3xl font-bold text-white font-mono text-center mb-1 tracking-tighter">{victim.name}</h2>
           <div className="flex items-center gap-2 text-zinc-500 font-mono text-xs uppercase tracking-[0.2em] mb-8">
           ...
           <div className="w-full space-y-4 mb-6">
               <div className="flex justify-between p-4 bg-zinc-900/50 rounded border-l-2 border-zinc-700 hover:border-green-500 transition-colors">
           ...
           <div className="w-full bg-zinc-900/30 p-4 rounded border border-zinc-800/50 mb-4">
        ...
        <div className="md:w-2/3 p-8 bg-zinc-900/20 flex flex-col h-full relative">
           <div className="mb-6 shrink-0">
               <h3 className="text-2xl font-bold text-white font-mono mb-1 flex items-center gap-3 tracking-tighter">
           ...
           <div className="flex-1 flex flex-col min-h-0">
                ...
                <div className="overflow-y-auto custom-scrollbar pr-2 grid grid-cols-2 gap-3 pb-2 content-start">
           ...
           {/* Footer Actions */}
           <div className="mt-6 pt-6 border-t border-zinc-800 flex justify-between items-center shrink-0 relative z-20">
               <button 
                onClick={onAbort}
                className="px-6 py-3 border border-red-900/30 text-red-500 hover:bg-red-950/50 hover:border-red-500/50 rounded font-mono text-xs font-bold flex items-center gap-2 transition-all uppercase tracking-wider"
               >
                   <AlertTriangle size={14}/> Abort Operation
               </button>

               <div className="flex items-center gap-4">
                   {loading && <span className="text-green-500 font-mono text-xs animate-pulse">ESTABLISHING UPLINK...</span>}
                   <button 
                    onClick={() => selectedCategory && onExecute(selectedCategory)}
                    disabled={!selectedCategory || loading}
                    className="px-10 py-4 bg-green-600 hover:bg-green-500 disabled:bg-zinc-900 disabled:text-zinc-700 disabled:cursor-not-allowed text-black font-bold rounded flex items-center gap-2 shadow-[0_0_30px_rgba(34,197,94,0.3)] transition-all hover:scale-105 hover:shadow-[0_0_50px_rgba(34,197,94,0.5)] font-mono tracking-tighter text-lg"
                   >
