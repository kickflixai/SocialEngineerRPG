...
const VictimDossier: React.FC<Props> = ({ victim, player, onExecute, loading }) => {
  ...
  const TraitBar = ({ label, value, color }: { label: string, value: number, color: string }) => (
      <div className="flex flex-col gap-1">
          <div className="flex justify-between text-[9px] font-mono uppercase tracking-wider text-zinc-500"><span>{label}</span><span className="text-white">{value}%</span></div>
          <div className="h-1.5 w-full bg-zinc-800 rounded-full overflow-hidden"><div className={`h-full ${color}`} style={{ width: `${value}%` }}></div></div>
      </div>
  );
      ...
      <div className="max-w-6xl w-full mx-auto bg-zinc-950 border-x md:border border-zinc-800/80 md:rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col md:flex-row relative">
        ...
        <div className="w-full md:w-1/3 bg-black/60 p-6 md:p-8 border-b md:border-b-0 md:border-r border-zinc-800 flex flex-col items-center relative shrink-0">
           ...
           <div className="w-full space-y-2 md:space-y-4 mb-4 md:mb-6">
               <div className="flex justify-between p-3 md:p-4 bg-zinc-900/50 rounded border-l-2 border-zinc-700 hover:border-green-500 transition-colors">
                   <span className="text-zinc-500 text-xs uppercase flex items-center gap-2 font-bold"><User size={12}/> Age</span><span className="text-white font-mono">{victim.age}</span>
               </div>
               <div className="flex justify-between p-3 md:p-4 bg-zinc-900/50 rounded border-l-2 border-zinc-700 hover:border-green-500 transition-colors">
                   <span className="text-zinc-500 text-xs uppercase flex items-center gap-2 font-bold"><Briefcase size={12}/> Role</span><span className="text-white font-mono text-right text-xs">{victim.occupation}</span>
               </div>
           </div>
           <div className="w-full bg-zinc-900/30 p-3 md:p-4 rounded border border-zinc-800/50 mb-4">
               <h4 className="text-zinc-400 text-xs font-bold uppercase mb-2 flex items-center gap-2 tracking-wider justify-center md:justify-start"><Fingerprint size={14} className="text-blue-500"/> Psych Profile</h4>
        ...
        <div className="w-full md:w-2/3 p-6 md:p-8 bg-zinc-900/20 flex flex-col h-auto relative items-center md:items-stretch text-center md:text-left">
           <div className="mb-6 shrink-0 w-full">
               <h3 className="text-xl md:text-2xl font-bold text-white font-mono mb-1 flex items-center justify-center md:justify-start gap-3 tracking-tighter"><Database size={24} className="text-green-500"/> INTEL_DATABASE</h3>
           ...
           <div className="grid grid-cols-1 gap-4 mb-8 shrink-0 w-full text-left">
               <div className={`p-5 rounded-xl border backdrop-blur-sm transition-all ${hasDoxxing ? 'bg-green-950/20 border-green-500/30' : 'bg-zinc-950/50 border-zinc-800'}`}>
                   <div className="flex justify-between items-start mb-3"><h4 className={`text-xs font-bold uppercase flex items-center gap-2 tracking-wider ${hasDoxxing ? 'text-green-400' : 'text-red-400'}`}>{hasDoxxing ? <Unlock size={14}/> : <Lock size={14}/>} Deep Web Leak</h4></div>
                   {hasDoxxing ? <p className="text-green-100 text-sm font-mono">{victim.hiddenFact}</p> : <p className="text-red-900/50 font-mono text-sm select-none tracking-widest">ENCRYPTED // NO ACCESS</p>}
           ...
           <div className="flex-1 flex flex-col min-h-0 w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pb-2">
                    {SCAM_CATEGORIES.map((cat) => (
                        <button key={cat} onClick={() => setSelectedCategory(cat)} className={`p-4 text-left rounded-lg border text-xs font-mono transition-all relative overflow-hidden group ${selectedCategory === cat ? 'bg-green-900/20 border-green-500 text-white shadow-[0_0_20px_rgba(34,197,94,0.2)]' : 'bg-zinc-900/40 border-zinc-800 text-zinc-400 hover:border-zinc-600 hover:text-zinc-200'}`}>
                            <div className={`absolute left-0 top-0 bottom-0 w-1 transition-all ${selectedCategory === cat ? 'bg-green-500' : 'bg-transparent group-hover:bg-zinc-700'}`}></div>{cat}
                        </button>
                    ))}
                </div>
           </div>
           ...
           <div className="mt-6 pt-6 border-t border-zinc-800 flex flex-col md:flex-row justify-end items-center gap-4 shrink-0 relative z-20 pb-8 md:pb-0 w-full">
               <div className="w-full md:w-auto flex items-center justify-center md:justify-end gap-4 order-1 md:order-2">
                   ...
                   <button onClick={() => selectedCategory && onExecute(selectedCategory)} disabled={!selectedCategory || loading} className="w-full md:w-auto px-8 md:px-10 py-4 bg-green-600 hover:bg-green-500 disabled:bg-zinc-900 disabled:text-zinc-700 disabled:cursor-not-allowed text-black font-bold rounded flex items-center justify-center gap-2 shadow-[0_0_30px_rgba(34,197,94,0.3)] transition-all hover:scale-105 hover:shadow-[0_0_50px_rgba(34,197,94,0.5)] font-mono tracking-tighter text-lg">
                       {loading ? <Loader2 size={20} className="animate-spin"/> : <Siren size={20}/>} INITIATE_HACK
                   </button>
               </div>
           </div>
        </div>
